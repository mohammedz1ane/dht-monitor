...
const CombinedChart = ({ data }) => {
     const [chartType, setChartType] = useState('temperature'); // 'temperature', 'humidity', 'both'
  ...
  const chartData = useMemo(() => {
      const labels = filteredData.map(item => new Date(item.dt).toLocaleTimeString());
       let datasets = [];
     if (chartType === 'temperature' || chartType === 'both') {
          datasets.push({
                label: 'Temperature (°C)',
       ...
       if (chartType === 'humidity' || chartType === 'both') {
            datasets.push({
                label: 'Humidity (%)',
   ...
   const options = {
        ...
        scales: {
            ...
            y: {
                title: {
                    display: true,
                   text: chartType === 'both' ? 'Value' : (chartType === 'temperature' ? 'Temperature (°C)' : 'Humidity (%)'),
                }
            }
        },
        plugins: {
            title: {
               display: true,
                text: 'Temperature and Humidity Variation',
            },
            tooltip: {
                callbacks: {
                    label: (context) => {
    ...
    return (
        <div>
            ...
            <div className="chart-buttons">
                <button onClick={() => handleChartTypeChange('temperature')} className={chartType === 'temperature' ? 'active' : ''}>Temperature</button>
                <button onClick={() => handleChartTypeChange('humidity')}  className={chartType === 'humidity' ? 'active' : ''}>Humidity</button>
                 <button onClick={() => handleChartTypeChange('both')}  className={chartType === 'both' ? 'active' : ''}>Both</button>
            </div>
